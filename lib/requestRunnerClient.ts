/**
 * Client-side request runner: runs in the browser so requests go from the user's
 * machine (e.g. to localhost APIs). No server proxy – ideal for local API testing.
 */
...
export async function runHttpRequestClient(
  reqDef: RequestDef,
  envVars: EnvVariable[]
): Promise<RunResult> {
  ...
  const timeoutMs = 60_000; // 60 seconds – avoid requests stuck forever
